    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                #         logger.info(f"Running eval over step {tr_nb}")
                #         results = evaluate(args, model, tokenizer, True, run, npt_logger, log_neptune)
                #         for key, value in results.items():
                #             logger.info("  %s = %s", key, round(value, 4))
                #             # Save model checkpoint
                #
                #     if results['eval_f1'] > best_metrics_es_batch:
                #         # early_stopping_count_batch = 0
                #         best_metrics_es_batch = results['eval_f1']
                #         logger.info("  " + "*" * 20)
                #         logger.info(f"  New Best F1 in step {tr_num}: {best_metrics_es_batch}", )
                #         logger.info("  " + "*" * 20)
                #
                ...
                #         logger.info("Saving model checkpoint to %s", output_dir)
                    ...
                    # else:
                    #     if early_stopping_count_batch >= max_early_stopping_count_batch:
                    #         break
                    #
                    #     early_stopping_count_batch += 1
                    #     logger.info("  " + "*" * 20)
                    #     logger.info(f"  F1 does not improve over batch number {tr_num},"
                    #                 f" early stopping counter: {early_stopping_count_batch}")
                    #     logger.info(f"  Current Batch Best F1: {best_metrics_es_batch}")
                    #     logger.info("  " + "*" * 20)
...
def main():
    ...
    if args.log_neptune:
        ...
        run = neptune.init_run(
            ...
            project="ntanh.gdsciu/reveal",
            api_token="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vYXBwLm5lcHR1bmUuYWkiLCJhcGlfdXJsIjoiaHR0cHM6Ly9hcHAubmVwdHVuZS5haSIsImFwaV9rZXkiOiIyZWQxNGNhNS0wY2I2LTQwNTktYjY3Yi1kMjM5MmM4NjQ2MjkifQ==",
        )
